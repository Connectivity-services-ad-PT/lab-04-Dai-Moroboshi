...
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    if isinstance(exc.detail, dict):
        ...
        problem = build_problem(
            ...
            title=http.HTTPStatus(exc.status_code).phrase if isinstance(exc.status_code, int) else "HTTP Error",
            detail=str(exc.detail),
            instance=str(request.url.path),
        )

    problem.setdefault("status", exc.status_code)
    problem.setdefault("title", http.HTTPStatus(exc.status_code).phrase if isinstance(exc.status_code, int) else "HTTP Error")
